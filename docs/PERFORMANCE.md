...
### C5-3（按连接 worker 卸载）判定为遗留项（2026-09-15）

**挂起，复活条件：selector 线程 CPU 饱和证据（>80%）。** C5-1/2 后的 58 采样归因显示
selector 线程仅 40–50%、锁链 0 BLOCKED、GC 排除——C5-3 并行化的是尚未饱和的容量，
单独实施预期收益 <10–20%。正确顺序：先做上述管线节奏小包（预期聚合 → 300MB/s 级）；
届时若 selector CPU 饱和，C5-3 从"无效手术"转为"必要扩容"，再做不迟。

### D1（管线节奏小包）两轮证伪后放弃（2026-09-15）

两个变体均低于 C5-1/2 基线，代码已回滚（同窗背靠背对比保证判定不受环境漂移影响）：

1. **到达即补发 + 深度 128**：请求粒度塌缩成单块一轮（17B 单帧 + 每块唤醒），
   曲线 -12~22%。附带修复过一个 refill 活锁（新选件全在途时 continue 热自旋）。
2. **半水位 64 块/批 + 深度 128**：同窗对比仍比 C5 的 16 块批低 25–36%，
   曲线压平成 1P≈8P≈58 的与 Peer 数无关的天花板。

**结论：补发触发粒度这一轴已证伪**——更深窗口（128）配任何批节奏都不如 C5 现状
（深度 32 + 16 块批）；多件指派 + 批量投递的每消息新增开销可能已超过摊薄收益。
若重启性能工作：先重置基线（重启机器复测，测量窗口曾出现与代码无关的 ~50% 整体压低），
再做单变量归因（仅加深度不动触发 / 仅批量投递不动深度）。

**当前有效性能状态 = C5-1/2：单 Peer 探针 140MB/s，曲线 162/172/184/139。**
